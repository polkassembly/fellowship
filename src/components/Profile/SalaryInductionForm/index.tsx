...
import SalaryInductionSuccessModal from './SalaryInductionSuccessModal';
...
function SalaryInductionForm({ address, formRef, setSubmitBtnText, isRegistration = false }: Props) {
	...
	const submitForm = async (e: FormEvent) => {
		...
		if (
			...
			address !== getSubstrateAddress(selectedAddress.address) ||
		...
		// if registration period && !inducted ? batch call of induct + register : induct

		const registrationPeriodDuration = new BN(api.consts.fellowshipSalary?.registrationPeriod?.toString?.() || 0);
		// eslint-disable-next-line @typescript-eslint/no-explicit-any
		const cycleStart = new BN(String(((await api.query.fellowshipSalary?.status?.())?.toJSON() as any)?.cycleStart));
		const currentBlockNumber = await getCurrentBlock(api);
		...
		if (!isInducted) {
			txArr.push(api.tx.fellowshipSalary?.induct());
		...
		if (isRegistrationPeriod) {
			txArr.push(api.tx.fellowshipSalary?.register());
			setSuccessDetails((prev) => {
				return {
					...prev,
					isRegistration: true
		...
		if (!txArr.length || claimant?.status === 'Registered') {
			...
			queueNotification({
				...
				message: 'This fellow address is already inducted and registered.',
				status: 'error'
			});
		...
		const tx = txArr.length > 1 ? api.tx.utility.batchAll(txArr) : txArr[0];
		...
		const onSuccess = async () => {
			queueNotification({
				...
				message: 'Salary induction successful.',
				status: 'success'
			});
	...
	if (successDetails.address) {
		return (
			<SalaryInductionSuccessModal
				address={successDetails.address}
				isInduction={successDetails.isInduction}
				isRegistration={successDetails.isInduction}
			/>
		);
	}
	...
	return (
		...
		>
			<div className='flex flex-col gap-2 text-xs'>
				<span className='text-xs capitalize'>Address To {isRegistration ? 'register' : 'induct'}:</span>
...
export default SalaryInductionForm;
