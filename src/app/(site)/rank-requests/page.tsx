...
export default async function RankRequestsPage({ searchParams }: ServerComponentProps<unknown, SearchParamProps>) {
	...
	const feedItems = await getActivityFeed({ feedType: EActivityFeed.RANK_REQUESTS, originUrl, network: network as Network });
	...
	return (
		<div className='flex w-full flex-col gap-y-8'>
			<div className='flex items-center rounded-[20px] border border-primary_border px-6 py-4'>
				...
				/>
				<div>
					<h3 className='text-xl font-semibold leading-6'>Rank Requests</h3>
				</div>
			</div>

			<VotingProposalsFeed
				feedType={EActivityFeed.RANK_REQUESTS}
				items={feedItems}
			/>
		</div>
	);
